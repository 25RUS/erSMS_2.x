...
#End Region
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        Try
            If TextBox2.Text <> "" Or ComboBox1.SelectedItem <> "" Or ComboBox2.SelectedItem <> "" Then
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        Connect()
        Try
            'My.Computer.FileSystem.WriteAllText("Plugins\erSMS_Resources\www\gate.html", "<html>" & vbNewLine & _
            '                                    "<head></head>" & vbNewLine & _
            '                                    "<body>", False)
            My.Computer.FileSystem.WriteAllText("Plugins\erSMS_Resources\www\gate.html", "", False)
            Dim adapter As SQLiteDataAdapter = New SQLiteDataAdapter("SELECT * FROM modems", m_dbConn)
            adapter.Fill(modem)
            For i = 0 To modem.Rows.Count - 1
                'ComboBox3.Items.Add(modem.Rows(i).Item(2))
                OpenPort(modem.Rows(i).Item(0), modem.Rows(i).Item(1))
                Dim OpSos As String = GetOperator()
                Dim SignalLevel As String = Signal()
                SignalLevel = "Plugins\erSMS_Resources\www\Signal\" + SignalLevel
                'генерирование таблицы
                My.Computer.FileSystem.WriteAllText("Plugins\erSMS_Resources\www\gate.html", "<table><tr><td>" & modem.Rows(i).Item(2) & "</td><td><table><tr><td>" & modem.Rows(i).Item(0) & "</td><td>" & modem.Rows(i).Item(1) & "</td><tr><td>" & OpSos & "</td></tr></table></td><td><img src=" & SignalLevel & "></mg></td></tr></table>", True)
                SP.Close()
            Next
            WebBrowser1.Navigate("Plugins\erSMS_Resources\www\gate.html")
        Catch ex As Exception
            MessageBox.Show("Error: " + ex.Message)
        End Try
    ...
    Private Function GetOperator()
        ...
        Catch ex As Exception
            'Dim D As Date = Now
            'My.Computer.FileSystem.WriteAllText(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\CLM\Plugins\erSMS_Resources\erSMS_log.txt", D & ": GetPort: " & ex.Message & vbNewLine, True)
        End Try
        Return GSMOperator
    End Function
    ...
    Private Function Signal()
        ...
        Try
            ...
            ' Dim D As Date = Now
            ' My.Computer.FileSystem.WriteAllText(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\CLM\Plugins\erSMS_Resources\erSMS_log.txt", D & " Signal: " & ModemAnswer & vbNewLine, True)
            'SP.Close()
        Catch ex As Exception
            'Dim D As Date = Now
        ...
        Try
            ...
            ElseIf SignalLevel1 < 0 Then
                ...
                Label3.ForeColor = Color.Green
            End If
            'PictureBox1.Load(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\CLM\Plugins\erSMS_Resources\Signal\" & img)
        Catch ex As Exception
            'Dim D As Date = Now
            'My.Computer.FileSystem.WriteAllText(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\CLM\Plugins\erSMS_Resources\erSMS_log.txt", D & ": Signal(indication error): " & ex.Message & vbNewLine, True)
        End Try
        Return img
    End Function
